...
def texto_puntuacion(frame, text, size, x,y):
    front = pygame.freetype.SysFont('Small Fonts', size, bold=True)
    text_frame, text_rect = front.render(text, True, negro)  
...
grupos_enemigos = pygame.sprite.Group()
...
while run:
    ...
    grupos_enemigos.update()
    grupo_balas_jugador.update()
    grupo_balas_enemigos.update()

    grupo_jugador.draw(window)

    # Coliciones balas_jugador - enemigos 
    colicion1 = pygame.sprite.groupcollide(grupos_enemigos, grupo_balas_jugador,True,True)
    for i in colicion1:
        score+=10
    ...
    # Coliciones jugador - balas_enemigos
    ...
    # Coliciones jugador - enemigos 
    hits = pygame.sprite.spritecollide(player, grupos_enemigos, False)
    for hit in hits:
        player.vida -= 100
        enemigo = Enemigos(10,10)
        grupo_jugador.add(Enemigos)
        grupos_enemigos.add(Enemigos)
        if player.vida <=0:
            run = False
